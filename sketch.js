...
function preload() {
  ...
  bgimg2 = loadImage("images/LightBackground.jpg");
...
function setup() {
  ...
  Map1 = createSprite(displayWidth / 2 + 200, displayHeight / 2 - 50);
...
function start() {
  ...
  introGreeting.position(displayWidth / 2 - 600, displayHeight / 2 - 350);
  introInput.value('');
  introInput.hide();
  introButton.hide();

  attackButton = createButton('Attack');
  attackButton.position(displayWidth / 2 - 20, displayHeight / 2 + 100)
  attackButton.mousePressed(attack);

  moneyText = createElement('h2', 'Money: ' + money);
  moneyText.position(displayWidth / 2 + 400, displayHeight / 2 - 350);
